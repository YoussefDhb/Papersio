...
class LaTeXPaperGenerator:
    ...
    def __init__(self, research_data: Dict[str, Any]):
        ...
        self._stopwords = {
            ...
            "were", "what", "when", "where", "which", "who", "why", "with", "about", "into",
            "between", "within", "without", "via", "using", "use", "used", "based", "across"
        }
    ...
    def _check_latex_installation(self):
        ...
        try:
            subprocess.run([self.pdflatex_cmd, '--version'], 
                         capture_output=True, check=True, timeout=5)
            subprocess.run([self.bibtex_cmd, '--version'], 
                         capture_output=True, check=True, timeout=5)
    ...
    def _parse_markdown_structure(self) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if header_match:
                ...
                elif 'introduction' in section_title or 'background' in section_title or 'related work' in section_title or 'literature' in section_title:
                ...
                elif 'conclusion' in section_title or ('summary' in section_title and sections.get('abstract')) or 'future work' in section_title:
    ...
    def _generate_bibtex(self) -> str:
        ...
        for idx, source in enumerate(self.sources, 1):
            ...
            if source_type == 'arxiv':
                ...
                access_note = self._escape_bibtex_text(
                    f"Accessed: {datetime.now().strftime('%Y-%m-%d')}"
                )
