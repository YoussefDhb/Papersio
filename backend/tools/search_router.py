...
def analyze_query_type(query: str) -> Dict[str, bool]:
    ...
    pure_academic_keywords = [
        'proof of', 'theorem', 'mathematical proof', 'formal verification',
        'equation derivation', 'theoretical framework'
    ]
    
    pure_web_keywords = [
        'price', 'cost', 'buy', 'download', 'tutorial', 'how to install',
        'stock market', 'weather', 'sports score'
    ]
    ...
    return {
        "search_arxiv": needs_arxiv,
        "search_web": needs_web,
        "reason": get_search_reason(needs_arxiv, needs_web)
    }
