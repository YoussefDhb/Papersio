...
class ResearchWorkflow:
    ...
    def __init__(self, planner: PlannerAgent, analyst: AnalystAgent, 
    ...
    async def execute_research(self, question: str, search_func, use_critique: bool = True) -> Dict:
        ...
        combined_context = "\n\n".join(all_contexts)
        workflow_stages["search"] = {
            "sub_results": search_results_list,
        ...
        if use_critique:
            ...
            while not critique.get("approved", True) and revision_count < max_revisions:
                ...
                await send_update(
                    "Revising",
                    f"Applying critique feedback (attempt {revision_count}/{max_revisions})..."
                )
