    ...
    def generate(self, prompt: str, temperature: float = 0.7, max_tokens: Optional[int] = None) -> str:
        ...
        try:
            ...
            response = self.client.models.generate_content(
                model=self.model_name, contents=prompt, config=self.types.GenerateContentConfig(**config)
            )
...
class ModelFactory:
    ...
    def create(cls, provider: str = None, model: str = None, **kwargs) -> LLMProvider:
        ...
        if provider not in cls.SUPPORTED_PROVIDERS:
            raise ValueError(
                f"Unsupported provider: {provider}. " f"Supported providers: {', '.join(cls.SUPPORTED_PROVIDERS.keys())}"
            )
