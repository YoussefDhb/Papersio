...
from tools.research_memory import save_research, get_research_context, save_paper_content, get_research_stats
...
def read_root():
    ...
    return {"message": "Papersio API is running!", "status": "ok", "version": "0.1.0"}
...
async def research_single_question(query: str, use_search: bool = True) -> Dict:
    ...
    if use_search:
        ...
        for paper in search_results["arxiv_results"]:
            sources.append(Source(title=paper["title"], url=paper["arxiv_url"], source_type="arxiv", authors=paper["authors"]))

        for article in search_results["web_results"]:
            sources.append(Source(title=article["title"], url=article["url"], source_type="web"))
...
Provide a clear, concise, and informative answer."""
    ...
    return {"answer": answer, "sources": sources, "search_strategy": search_strategy}
        ...
        if request.use_planning:
            ...
            if plan.get("needs_breakdown") and plan.get("sub_questions"):
                for sub_q in plan["sub_questions"]:
                    ...
                    sub_results.append(
                        SubQuestionResult(question=sub_q, answer=sub_result["answer"], sources=sub_result["sources"])
                    )

                    all_sources.extend(sub_result["sources"])

                sub_qa_text = "\n\n".join([f"Q: {sr.question}\nA: {sr.answer}" for sr in sub_results])
...
async def research_workflow(request: ResearchRequest):
    ...
    try:
        ...
        async def search_function(query: str):
            ...
            for paper in search_results["arxiv_results"]:
                sources.append(
                    Source(title=paper["title"], url=paper["arxiv_url"], source_type="arxiv", authors=paper["authors"])
                )

            for article in search_results["web_results"]:
                sources.append(Source(title=article["title"], url=article["url"], source_type="web"))

            return {"sources": sources, "context": search_results["combined_context"]}

        result = await workflow.execute_research(request.query, search_function, use_critique=True)
        ...
        raise HTTPException(status_code=500, detail=f"Error in workflow: {error_msg}")
...
async def research_langgraph(request: ResearchRequest):
    ...
    try:
        ...
        async def search_function(query: str):
            ...
            for paper in search_results["arxiv_results"]:
                sources.append(
                    {"title": paper["title"], "url": paper["arxiv_url"], "source_type": "arxiv", "authors": paper["authors"]}
                )

            for article in search_results["web_results"]:
                sources.append({"title": article["title"], "url": article["url"], "source_type": "web"})

            return {"sources": sources, "context": search_results["combined_context"]}

        langgraph_workflow = LangGraphResearchWorkflow(
            planner=planner, analyst=analyst, writer=writer, critic=critic, search_func=search_function
        )

        final_state = await langgraph_workflow.execute(query=request.query, use_search=request.use_search)

        formatted_sources = []
        for source in final_state.get("sources", []):
            formatted_sources.append(
                Source(
                    title=source["title"], url=source["url"], source_type=source["source_type"], authors=source.get("authors")
                )
            )
        ...
        if final_state.get("plan"):
            workflow_stages.append({"stage": "Planning", "status": "complete", "details": final_state["plan"]})

        if final_state.get("sources"):
            workflow_stages.append(
                {"stage": "Search", "status": "complete", "details": f"Found {len(final_state['sources'])} sources"}
            )
        ...
        raise HTTPException(status_code=500, detail=f"Error in LangGraph execution: {error_msg}")
    ...
    try:
        ...
        if request.use_search:
            arxiv_results = search_arxiv(
                request.query, max_results=2, extract_full_content=True  # Limit to 2 papers to avoid timeout
            )
        ...
        raise HTTPException(status_code=500, detail=f"Error in contextual research: {error_msg}")
    ...
    try:
        while True:
            ...
            async def on_step_update(step_name, step_details=None):
                await manager.send_message({"type": "status", "stage": step_name, "details": step_details}, str(id(websocket)))
            ...
            try:
                if mode == "ultra":
                    ...
                    await manager.send_message(
                        {"type": "result", "answer": result.get("final_report", ""), "sources": result.get("sources", [])},
                        str(id(websocket)),
                    )
                    ...
                    await manager.send_message(
                        {"type": "result", "answer": result["final_report"], "sources": result["sources"]}, str(id(websocket))
                    )
...
async def research_ultra(request: ResearchRequest):
    ...
    try:
        ...
        async def ultra_search_function(query: str):
            ...
            if not request.use_search:
                return {"sources": [], "context": "", "papers_extracted": 0, "tables_found": 0}

            print(f" Searching ArXiv with PDF extraction...")
            arxiv_results = search_arxiv(query, max_results=2, extract_full_content=True)
        ...
        final_state = await ultra_workflow.execute(query=request.query, use_search=request.use_search)
        ...
        workflow_stages = [
            ...
            {"stage": "Planning", "status": "complete", "details": final_state.get("plan", {})},
            {
                "stage": "Search",
                "status": "complete",
                "details": {
        ...
        raise HTTPException(status_code=500, detail=f"Error in Ultra execution: {error_msg}")


@app.post("/api/research/export/pdf")
def export_research_pdf(research_data: dict, citation_style: str = "IEEE"):
    """
    ...
    try:
        ...
        if not research_data.get("query") or not research_data.get("answer"):
            raise HTTPException(status_code=400, detail="Missing required fields: query and answer")
        ...
        if "LaTeX not found" in error_msg:
            ...
            raise HTTPException(status_code=500, detail=f"PDF generation error: {error_msg}")

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Unexpected error during PDF export: {str(e)}")
...
def get_stats():
    ...
    try:
        ...
        return {"database": stats, "vectors": vector_stats, "message": "Research memory statistics"}
    except Exception as e:
